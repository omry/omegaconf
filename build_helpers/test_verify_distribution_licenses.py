...
def test_read_antlr_license_from_legacy_wheel_layout(tmp_path: Path) -> None:
    wheel = tmp_path / "omegaconf-1.0-py3-none-any.whl"
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr("omegaconf-1.0.dist-info/LICENSE-antlr4", LICENSE_TEXT)
...
def test_non_file_antlr_license_is_rejected_from_sdist(tmp_path: Path) -> None:
    sdist = tmp_path / "omegaconf-1.0.tar.gz"
    with tarfile.open(sdist, "w:gz") as archive:
        member = tarfile.TarInfo(f"omegaconf-1.0/{LICENSE_PATH}")
    ...
    with pytest.raises(RuntimeError, match="ANTLR license is not a file"):
        read_antlr_license(sdist)
