...
and `"error"` raises `TypeError`. The default is `"warn"` in 2.4 and `"error"`
in 2.5; explicit modes behave identically in both releases. Other values are
invalid.
...
2. Bind them to the signature and validate annotated ordinary and variadic
   parameters.
