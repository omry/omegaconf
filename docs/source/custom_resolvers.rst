...
- ``"error"`` raises an error on a mismatch.
...
argument interpolations are evaluated and before resolver cache lookup. Return
validation occurs before a new result is cached and also checks cache hits.
