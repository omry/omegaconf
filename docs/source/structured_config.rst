...
A value of the declared type can be assigned directly. Direct assignment of a
convertible value still works in 2.4, but emits a ``FutureWarning``. Use
``OmegaConf.update()`` to request conversion explicitly without a warning.
