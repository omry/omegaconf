...
.. warning::
    ...
    are involved, results may depend on key insertion order, because custom
    resolvers can do anything — they may be stateful, have side effects, or return
    different values on each call.  No traversal strategy can account for this, so
