...
def test_enum_to_str() -> None:
    ...
    container: Dict[str, Any] = OmegaConf.to_container(cfg, enum_to_str=False)  # type: ignore
    container_enum_to_str: Dict[str, Any] = OmegaConf.to_container(cfg, enum_to_str=True)  # type: ignore

    # list
    assert container["l"][0] == Color.RED
    assert container_enum_to_str["l"][0] == "RED"

    # dict keys
    assert list(container["d"])[0] == Color.RED
    assert list(container_enum_to_str["d"])[0] == "RED"

    # dict values
    assert container["d"]["red"] == Color.RED
    assert container_enum_to_str["d"]["red"] == "RED"
