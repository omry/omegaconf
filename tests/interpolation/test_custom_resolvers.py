...
def test_register_resolver_twice_error_not_same_cache_setting(
    restore_resolvers: Any, first_use_cache: bool
) -> None:
    def foo(_: Any) -> int:
        return 10
    ...
    with raises(ValueError, match=re.escape("resolver 'foo' is already registered")):
        OmegaConf.register_new_resolver("foo", foo, use_cache=not first_use_cache)


@mark.parametrize("use_cache", [True, False])
def test_register_resolver_twice_ok(restore_resolvers: Any, use_cache: bool) -> None:
    def foo(_: Any) -> int:
        return 10

    OmegaConf.register_new_resolver("foo", foo, use_cache=use_cache)
    OmegaConf.register_new_resolver("foo", foo, use_cache=use_cache)
