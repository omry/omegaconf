...
class TupleConfig(BaseContainer, Sequence[Any]):
    ...
    def _unresolved_nodes(self) -> List[Node]:
        ...
        if self._is_missing():
            raise MissingMandatoryValue("Cannot operate on a missing TupleConfig")
        if self._is_interpolation():
            raise InterpolationResolutionError(
                "Cannot operate on an unresolved TupleConfig interpolation"
            )
    ...
    def __getitem__(self, key_or_index: Union[int, slice]) -> Any:
        ...
        try:
            ...
            if self._is_interpolation():
                raise InterpolationResolutionError(
                    "Cannot index an unresolved TupleConfig interpolation"
                )
    ...
    def _iter_ex(self, resolve: bool) -> Iterator[Any]:
        try:
            ...
            if self._is_interpolation():
                raise InterpolationResolutionError(
                    "Cannot iterate an unresolved TupleConfig interpolation"
                )
            return TupleConfig.TupleIterator(self, resolve)
        except (TypeError, MissingMandatoryValue, InterpolationResolutionError) as ex:
    ...
    def __mul__(self, count: Any) -> "TupleConfig":
        ...
        content = self._unresolved_nodes()
        if count <= 0:
            return self._new_with_fixed_type([], ())
    ...
    def _set_value_impl(
        ...
        if not isinstance(value, (list, tuple, ListConfig, TupleConfig)):
            raise ValidationError(
                f"Invalid value assigned: {type(value).__name__} is not a "
                "TupleConfig, ListConfig, list, or tuple."
            )
